...
function removeIngressRuleIfNecessary(funcName, namespace, options) {
  ...
  return new BbPromise((resolve, reject) => {
    extensions.ns.ingress.get((err, ingressInfo) => {
      ...
      if (!_.isEmpty(ingressRule)) {
        ...
        if (opts.verbose) {
          opts.log(`Skipping ingress rule clean up for ${funcName}`);
        }
        resolve();
      }
    });
      ...
      thirdPartyResources.ns.functions.delete(desc.id, (err) => {
        if (err) {
          ...
          .then(() => {
            ...
            opts.log(`Function ${desc.id} succesfully deleted`);
          });
        }
      });
    });
