...
function getFunctionDescription(
  ...
  if (image || env || memory || secrets || cpu || affinity || tolerations || volumes) {
    ...
    if (volumes !== undefined && volumes.length > 0) {
        createVolumes(container, funcs, volumes)
    }
...
function createVolumes(container, funcs, volumes) {
    ...
    volumes.forEach(volume => {
      ...
      volumeType.name = volume.name;
        container.volumeMounts.push({ name: volume.name, mountPath: volume.mountPath });
        funcs.spec.deployment.spec.template.spec.volumes
            .push(volumeType);
    });
}
