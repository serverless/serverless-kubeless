...
function getFunctionDescription(
  ...
  if (volumes){
    ...
    // volumes.forEach(volume => {
      ...
      azureDisk: {        
        kind: 'Managed', 
        diskName: 'Tools-Volume',
        diskURI: '/subscriptions/543bce2b-aa4c-45a9-8058-15c6905b083a/resourceGroups/MC_experimental_experimental-ageas-aks_uksouth/providers/Microsoft.Compute/disks/Tools-Volume'
        }
      }
    );
    // });
  ...
  if (image || env || memory || secrets || cpu || affinity || tolerations || volumes) {
    ...
    if (volumes !== undefined) {
      console.log('There are some volumes');
      console.log(volumes);
      if (container.volumeMounts === undefined) {
        container.volumeMounts = [];
      }
      volumes.forEach(volume => {
        // container.volumeMounts.push({ name: `${volume.name}-vol`, mountPath: `/${volume.name}` });
        container.volumeMounts.push({ name: `Azure-vol`, mountPath: `/mnt/azure`});
      });
    }
  }
  return funcs;
