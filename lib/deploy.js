...
function deployMQTrigger(mqType, name, namespace, topic, options) {
  ...
  const triggerApi = new CRD('apis/kubeless.io', 'v1beta1', namespace, mqType.toLowerCase() + 'triggers');
...
function deployTrigger(event, funcName, namespace, service, options) {
  ...
  switch (event.type) {
    ...
    case 'trigger':
      ...
      triggerPromise = handleMQTDeployment(event.trigger, funcName, namespace, { log: options.log });
      break;
    case 'schedule':
